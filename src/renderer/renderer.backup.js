...
console.log('[Forge Renderer] Script loaded at', new Date().toISOString());

class ForgeBrowser {
  constructor() {
    console.log('[Forge Renderer] Constructor called');
    this.tabs = [];
  ...
  createTab(url = null) {
    ...
    webview.addEventListener('did-navigate', (e) => {
      ...
      console.log('[Password DEBUG] About to call injectPasswordAutofill from did-navigate');
      try {
        this.injectPasswordAutofill(webview, e.url);
      } catch (pwErr) {
        console.error('[Password DEBUG] Error calling injectPasswordAutofill:', pwErr);
      }
    });
    ...
    webview.addEventListener('dom-ready', () => {
      try {
        ...
        if (url) {
          ...
          console.log('[Password DEBUG] About to call injectPasswordAutofill from dom-ready');
          try {
            this.injectPasswordAutofill(webview, url);
          } catch (pwErr) {
            console.error('[Password DEBUG] Error calling injectPasswordAutofill:', pwErr);
          }
        }
  ...
  async showPasswordAnvil() {
    await window.forgeAPI.createPasswordAnvilWindow();
  }
  ...
  async injectPasswordAutofill(webview, url) {
    console.log('[Password] injectPasswordAutofill called with URL:', url);
    ...
    console.log('[Password] Attempting to inject autofill script for:', url);
    
    try {
      // Set up listener BEFORE injecting script
      this.setupPasswordRequestListener(webview);
      
      // Build script using regular strings to avoid template literal escaping issues
      const script = [
        '(function() {',
        '  "use strict";',
        '  ',
      ...
      console.log('[Password] Autofill script injected successfully for', url);
    } catch (e) {
      console.error('[Password] Failed to inject autofill script:', e.message, e.stack);
    }
  }
  
  setupPasswordRequestListener(webview) {
    // Check if listener is already set up
    if (webview._passwordListenerSetup) {
      return;
    }
    webview._passwordListenerSetup = true;
    ...
    webview.addEventListener('console-message', async (e) => {
      if (e.message && e.message.startsWith('[FORGE_REQUEST_PASSWORDS]')) {
        ...
        console.log('[Password] Request for passwords at:', url);
        
        try {
          // Get passwords for this URL
          const passwords = await window.electronAPI.passwords.getForUrl(url);
          
          console.log('[Password] Found', passwords ? passwords.length : 0, 'passwords');
          
          // Send passwords back to webview
          if (passwords && passwords.length > 0) {
            // Remove sensitive data from logs
            const sanitized = passwords.map(p => ({ ...p, password: '***' }));
            console.log('[Password] Sending credentials:', sanitized);
            
            const code = `
              (function() {
                console.log('[Forge] Receiving passwords from host');
                window.postMessage({
                  type: 'FORGE_PASSWORDS',
